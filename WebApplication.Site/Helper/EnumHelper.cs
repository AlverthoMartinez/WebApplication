...
namespace WebApplication.Site.Helper
{
	...
	public static class EnumHelper
	{
        ...
        {
            Type realModelType = obj.GetType();
		...
		// 	return typeof(Resources.Resources);
		// }
		...
		{
			// precedence:
            // Gets value from resource file in [Display] attribute
            // Gets value from resource file
			// if null, gets value from [Description] attribute
			// if null, gets value.ToString()


            // if no resource specified, tries to find [LocalizationEnum] or uses Resources.Resources
            // if (resourceType == null)
            //     resourceType = GetResourceType<TEnum>();

            Type type = GetUnderlyingType(value);
			string resourceKey = string.Format("{0}.{1}", type.Name, value.ToString());

            var enumValue = Enum.GetName(type, value);
            MemberInfo member = type.GetMember(enumValue)[0];
            var displayAttr = (DisplayAttribute)member.GetCustomAttribute(typeof(DisplayAttribute), false);
            if (displayAttr != null)
            {
                resourceType = displayAttr.ResourceType;
                resourceKey = displayAttr.Name;
            }
        ...
        /// <summary>
        /// Obtains the value string of the DisplayName attribute for the enum.
        /// The display name is obtained from the following sources:
        ///     1. Resource type and key specified by the DisplayAttribute. If no resource type is specified,
        ///         it uses Resources.Resoucres. If no resource key is specified, it uses EnumType.EnumFieldName
        ///         format.
        ///     2. Name from the DisplayAttribute.
        ///     3. The name of the enum field.
        /// </summary>
        /// <param name="value">Enum Value</param>
        /// <returns>Enum Description</returns>
        public static string DisplayName(Enum value)
        {
            return GetLocalizedString(null, value);
        }
        ...
        /// <param name="value">Enum Value</param>
        /// <returns>Enum Description</returns>
        public static string DisplayName<E>(E value) 
